    ...
    try:
        ...
        try:
            ...
            for (szemelyi_szam, nev) in cur:
                results.append({'szemelyi_szam': szemelyi_szam,
                               'nev': nev})
    ...
    try:
        ...
        try:
            ...
            for (poseidonkod, nev, szuletesidatum) in cur:
                results.append({'PoseidonKod': poseidonkod, 'Nev': nev,
                               'SzuletesiDatum': date.isoformat(szuletesidatum)})
...
def list_student_bypos_county(pos):
    ...
    try:
        cur = conn.cursor()
        try:
            cur.execute('SELECT NEV,SZULETESIDATUM,CIM,EGYETEMKEZDESEVE FROM HALLGATOK WHERE POSEIDONKOD=:pkod'
                        , pkod=pos)
            result = cur.fetchone()
            if result is None:
                abort(404)
            ...
            if request.method == 'GET':
                ...
                try:
                    ...
                    return jsonify(Nev=result[0],
                                   ...
                                   Cim=result[2],
                                   EgyetemKezdesEve=result[3],
                                   Kisterseg='Ismeretlen')
            if request.method == 'DELETE':
                cur.execute('DELETE FROM HALLGATOK WHERE POSEIDONKOD=:pkod'
                            , pkod=pos)
                conn.commit()
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                params = {  # 2012-04-01 -> "April 01" -> "April 1"
                    'action': 'query',
...
def verb_test():
    ...
    return jsonify(method=request.method, data=request.get_json(),
                   url=request.url)
...
def get_db():
    ...
    return cx_Oracle.connect(config['user'], config['pass'],
                             config['host'])
